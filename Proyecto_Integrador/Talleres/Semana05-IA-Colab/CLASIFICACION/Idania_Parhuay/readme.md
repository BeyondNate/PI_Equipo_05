...
- Exclusión de Daily Obs Count por estar casi perfectamente correlacionada con Percent Complete (r ≈ 1.00 en esta base de datos), lo que generaba multicolinealidad severa; se conservó Percent Complete por ser la variable estándar de completitud reportada por la EPA.
...
La concentración media de CO en 2022 fue de 0.30 ppm, con una desviación estándar de 0.15 ppm, un valor mínimo de 0.10 ppm y un máximo de 1.30 ppm. La mediana (0.30 ppm) es prácticamente igual a la media, pero el histograma de la Figura 1 muestra una distribución con marcada asimetría positiva: la mayoría de los días se concentran entre 0.2 y 0.3 ppm, mientras que existe una cola derecha de días con valores más altos, asociados probablemente a episodios puntuales de mayor tráfico vehicular, condiciones meteorológicas de baja dispersión atmosférica (inversión térmica) o quema de biomasa. La curva de densidad (Figura 2) confirma este patrón, mostrando un pico pronunciado en valores bajos y una cola larga hacia valores mayores.
...
El RMSE del modelo (0.1411 ppm) es del mismo orden que la propia variabilidad de la concentración de CO, lo que es consistente con un modelo que no logra explicar la variabilidad de la variable objetivo. Esto se traduce en un R² negativo (-0.0133), lo que significa que, sobre los datos de prueba, el modelo lineal ajustado predice ligeramente peor que una predicción constante igual a la media del conjunto de entrenamiento.
...
La Figura 5 ilustra claramente esta limitación: las predicciones del modelo (eje vertical) se concentran en una banda muy estrecha, prácticamente independiente del valor real observado (eje horizontal). En otras palabras, el modelo se comporta de forma muy similar a una predicción constante cercana a la media histórica, y es incapaz de anticipar los días con concentraciones elevadas.
...
Esta falta de poder explicativo tiene una explicación razonable desde el punto de vista de la química atmosférica: la concentración de CO en aire ambiental depende principalmente de fuentes de combustión (tráfico vehicular, quema de biomasa, actividad industrial) y de condiciones meteorológicas que favorecen o dificultan la dispersión de contaminantes (velocidad y dirección del viento, temperatura, presión atmosférica, inversiones térmicas), ninguna de las cuales está representada en las variables utilizadas. Las variables temporales (mes, día del año, día de la semana) son, en el mejor de los casos, proxies muy indirectos de patrones estacionales o de actividad humana, y su correlación prácticamente nula con la variable objetivo, visible en la Figura 3, así lo confirma. De manera similar, las variables de completitud de las observaciones (Percent Complete, Daily Obs Count) son indicadores de la calidad del proceso de medición, no de la magnitud del fenómeno medido, por lo que resulta razonable que aporten muy poco valor predictivo; a esto se suma que, en esta base de datos, dichas variables presentan muy poca variabilidad real (el 75 % de los días alcanzó 100 % de completitud), lo que limita aún más su capacidad de discriminar entre días con distintos niveles de CO.

Un hallazgo relevante del proceso metodológico fue la detección de una correlación casi perfecta entre Daily Obs Count y Percent Complete (r ≈ 1.00), por lo que se decidió excluir Daily Obs Count del conjunto de predictores y conservar Percent Complete como variable estándar de completitud, evitando así problemas de multicolinealidad en el modelo final. El número de condición del modelo OLS resultante (6.88 × 10³) no muestra evidencia de multicolinealidad severa entre los predictores utilizados, lo que confirma que el bajo desempeño del modelo no se debe a problemas de colinealidad, sino a la ausencia de una relación lineal genuina entre los predictores disponibles y la variable objetivo. Este resultado ilustra un punto metodológico importante: prevenir problemas de diseño del modelo (como la multicolinealidad) es necesario para una interpretación correcta de los coeficientes, pero no garantiza, por sí solo, una mejora sustancial en la capacidad predictiva cuando el conjunto de variables carece de señal explicativa relevante.
